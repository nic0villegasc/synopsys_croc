...
set REPORT_PREFIX ${COMMIT_BLOCK_BLOCK_NAME}
...
if {$DP_HIGH_CAPACITY_MODE != "true"} {
  ...
  # To avoid losing out on multicycle path constraints that are set on internal logical hierarchy pins of child blocks
  # By default, tool assumes that the blocks are represented by abstracts and therefore skips retention of internal constraints (that are not visible on the boundary of the abstracts) in the top constraints file
  set_app_options -list {plan.budget.all_design_subblocks true}

  ## This is a workaround for STAR 3359010.  The default becomes "true" in 22.03 release.
  set_app_options -list {mv.hierarchical.split_constraints_exclude_physical_blocks true}

  puts "Info : Splitting constraints"
...
########################################################################
## Commit blocks and load constraints
########################################################################
...
foreach ref ${DP_BLOCK_REFS} {
  ...
  set_attribute -object ${target_design_lib} -name use_hier_ref_libs -value true ;
}
