...
set REPORT_PREFIX ${CREATE_POWER_FLAT_BLOCK_NAME}
...
copy_block -from ${DESIGN_NAME}/${CREATE_FLOORPLAN_FLAT_BLOCK_NAME} -to ${DESIGN_NAME}/${CREATE_POWER_FLAT_BLOCK_NAME}
current_block ${DESIGN_NAME}/${CREATE_POWER_FLAT_BLOCK_NAME}
...
if { [lsearch $operations "PNS"] >=0 } {
  ...
  if {[get_attribute -quiet [current_block] port_placement_export_file] != ""} {
    ...
    if {[sizeof_collection $port_list]} {
...
if { [lsearch $operations "STDCELL_PLACEMENT"] >=0 } {
  ...
  ## - Note that macros were fixed in create_floorplan.tcl
  ####################################

  rm_source -file $TCL_USER_STDCELL_PLACEMENT_PRE_SCRIPT -optional -print "TCL_USER_STDCELL_PLACEMENT_PRE_SCRIPT"

  ## Reset plan.macro.macro_place_only as it was set true in macro placement.
  reset_app_options plan.macro.macro_place_only
  ...
  save_block -as ${DESIGN_NAME}/stdcell_placement
