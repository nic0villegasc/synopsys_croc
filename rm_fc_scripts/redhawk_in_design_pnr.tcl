...
if {$REDHAWK_SCRIPT_FILE == "" && $REDHAWK_MCMM_SCENARIO_CONFIG == ""} {
	if {$REDHAWK_SWITCHING_ACTIVITY_FILE == ""} {
		if {$REDHAWK_USE_FC_POWER} {
			if {($REDHAWK_ANALYSIS == "static" || $REDHAWK_ANALYSIS == "dynamic_vcd" || $REDHAWK_ANALYSIS == "dynamic_vectorless" || $REDHAWK_ANALYSIS == "dynamic" ) && !$REDHAWK_EM_ANALYSIS} {
				...
				analyze_rail -nets $REDHAWK_ANALYSIS_NETS -voltage_drop $REDHAWK_ANALYSIS -extra_gsr_option $extra_gsr_file -power_analysis icc2
                        	...
                        	analyze_rail -nets $REDHAWK_ANALYSIS_NETS -voltage_drop $REDHAWK_ANALYSIS -electromigration  -extra_gsr_option $extra_gsr_file -power_analysis icc2
...
if {$REDHAWK_PGA_NODE != ""} {
	...
	if {$REPORT_QOR} {
		...
		file delete -force ${REPORTS_DIR}/${REPORT_PREFIX}
                sh mkdir ${REPORTS_DIR}/${REPORT_PREFIX}
