/*******************************************************************************************************************
*
*      FILE NAME       : W25Q128JWxxIM.v
*      DESCRIPTION     : Verilg TestBehch example of Winbond Electronics Corp. series Serial Flash Memory
*      VERSION         : Rev 1.0 - Initial Release
*      RELEASE         : MAY, 2018
*      PART NUMBER     : W25Q128JWxxIM
*      MEMORY DENSITY  : 128Mb
*      Max.SPEED       : 133MHz
*      AUTHOR          : Perseus Hsu
*
*                                                 Copyright(c)  Winbond Electronics Corp., 2018. All rights reserved.
*******************************************************************************************************************/

`include "./W25Q128JWxxIM_V1.1_Incisive.vp"
/******************************************************************************
 TEST VECTORS
******************************************************************************/

module test_W25Q128JWxxIM;

W25Q128JWxxIM dut(CSn, CLK, DIO, DO, WPn, HOLDn);

reg  CSn_Reg, CLK_Reg;

parameter PAGESIZE = 256;
parameter NUM_PAGES = 32768;
...
// Initialize all registers
	...
	spi_sr(status);
        ...
        $display("Erase Block 0, with Suspend / Resume");
...
begin
   ...
   spi_resume;
	...
	$display("Erase block with Suspend / Resume (Address = %h)",address);

   //$display("First sector dump should be pre erase data. Second sector dump should be erased data.");
   $display("First block dump should be pre erase data. Second block dump should be erased data.");
