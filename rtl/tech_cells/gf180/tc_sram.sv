...
module tc_sram #(
  ...
  if (NumWords == 64 && DataWidth == 64 && P1L1) begin: gen_64x64xBx1
    ...
    // GF180 uses 512x8 macros. 
    // We instantiate 4 of them in parallel to achieve a 32-bit width.
    for (genvar i = 0; i < 4; i++) begin : gen_sram_banks
      gf180mcu_fd_ip_sram__sram512x8m8wm1 i_sram_bank (
          .CLK  ( clk_i ),
          .CEN  ( ~req_i[0] ),             // Active Low Chip Enable
          .GWEN ( ~we_i[0] ),              // Active Low Global Write Enable
          .WEN  ( {8{~be_i[0][i]}} ),      // Byte-enable expanded to 8 active-low bits
          .A    ( addr_i[0][8:0] ),        // 9-bit address bus shared across all banks
          .D    ( wdata_i[0][i*8 +: 8] ),  // 8-bit slice of the 32-bit input data
          .Q    ( rdata_o[0][i*8 +: 8] ),  // 8-bit slice of the 32-bit output data
          .VDD  ( 1'b1 ),                  // Power pin
          .VSS  ( 1'b0 )                   // Ground pin
      );
    end
